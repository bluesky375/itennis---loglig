(function () {
	angular.module("LogligApp.controllers")
		.controller("trainingTeamCtrl", ["$scope", "$state", "$ionicScrollDelegate", "$ionicSlideBoxDelegate", "$timeout", "appService", "teamsService", "clubsService", "$location", "$ionicPopover", "authService", "$http", "appUtilsService", "unionService", "$translate", "multimediaService", "$q", "$cordovaFileTransfer",
				...
				$scope.bindTeam = function () {
					...
					teamsService.getClubTeamData(teamId, clubId).then(function (res) {
						if (res.data != null) {
							...
							// console.log("checkmeplayersList" + JSON.stringify(res.data.Players));
