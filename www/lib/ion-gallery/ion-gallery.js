...
(function(){
  ...
  ionSlider.$inject = ['$ionicModal', 'ionGalleryHelper', '$ionicPlatform', '$timeout', '$ionicScrollDelegate', 'apiService', '$ionicPopup', '$translate', 'appService'];

  function ionSlider($ionicModal, ionGalleryHelper, $ionicPlatform, $timeout, $ionicScrollDelegate, apiService, $ionicPopup, $translate, appService) {
    ...
    function link(scope, element, attrs) {
      ...
      scope.report = function(single) {
        _modal.hide();
        apiService.makeRequest({
          url: API_REPORT_GALLERY,
          method: "POST",
          data: {
            Sender: single.User.Name,
            Date: single.Created,
            SenderId: single.User.Id,
            ImgName: single.url,
            Type: single.type,
            ParentId: single.ParentId,
            ParentName: single.ParentName
          }
        }).then(function () {
          //appService.alertPopup($translate.instant("mail_sent_successful"));
        }, function (err) {
          // $ionicPopup.alert("Error during reporting");
        });
      };

      scope.removeImage = function (single) {
        _modal.hide();
        var tt = single.type == "team" ? "teams" : "leauges";
        var url = BASE_API_URL + "/api/" + tt + "/DeleteGallery/" + single.url;
        apiService.getByUrl(url).then(function (data) {
