(function () {
	angular.module("LogligApp.controllers")
		.controller("leagueTeamCtrl", ["$scope", "$state", "$ionicScrollDelegate", "$ionicSlideBoxDelegate", "$timeout", "appService", "teamsService", "leaguesService", "$location", "$ionicPopover", "authService","$http" ,"unionService", "$filter", "multimediaService", "$q", "$cordovaFileTransfer", "$translate",
			function ($scope, $state, $ionicScrollDelegate, $ionicSlideBoxDelegate, $timeout, appService, teamsService, leaguesService, $location, $ionicPopover, authService, $http, unionService, $filter, multimediaService, $q, $cordovaFileTransfer, $translate) {

				var leagueId = $state.params.leagueId;
				var teamId = $state.params.teamId;
				$scope.lId = leagueId;
				$scope.tId = teamId;
				$scope.slideGame = null;
				$scope.gameSlideIndex = -1;
				...
				$scope.gamesHander = teamsHandler;
				...
				$scope.getSlideGameFans = function (game) {
				...
				$scope.gamesSlideChanged = function (num) {
					...
					$scope.gameSlideIndex = num;
					$scope.slideGame = game;
					$scope.slideGame.IsGoingFlag = game.IsGoing;
					$scope.cycleNumber = game.CycleNumber;
					$scope.getSlideGameFans(game);
				...
				$scope.bindTeamData = function () {
					...
					teamsService.getLeagueTeamData(teamId, leagueId).then(function (res) {
						if (res.data != null) {
							$scope.team = res.data.TeamInfo;
							$scope.team.bFromTeamPage = true;
							$scope.leagues = {
							...
							if (res.data.NextGame) {
								...
								if (teamsHandler._instances.length) {
				...
				$scope.onChangeLigue = function (Id) {
					$state.go('app.leagueteam', {teamId: $scope.team.TeamId, leagueId: Id});
				};
				//// init page view
				$scope.bindTeamData();
                ...
                $scope.onClickStat = function() {
					$state.go("app.league", ({leagueId: $scope.team.LeagueId}));
				};
				...
				$scope.$watch('slideGame.IsGoingFlag',function(newvalue, oldvalue){
					if(typeof(newvalue)!== 'undefined' && typeof(oldvalue) !== 'undefined' && newvalue !== oldvalue){
						$scope.getSlideGameFans($scope.slideGame);
					}
				});
